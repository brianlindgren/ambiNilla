...
"""
3rd-Order Ambisonics Stereo Decoder Generator
---------------------------------------------
Generates a pseudo-inverse decoding matrix for a 2-speaker stereo array
(±45° on the horizontal plane), using ACN channel ordering with either
N3D (orthonormalized) or SN3D (Schmidt semi-normalized) spherical harmonics.

USAGE:
    python ambi_decoder_stereo.py
        -> Generates both 3OA_Stereo_SN3D.txt and 3OA_Stereo_N3D.txt

    python ambi_decoder_stereo.py --norm SN3D
        -> Generates only 3OA_Stereo_SN3D.txt

    python ambi_decoder_stereo.py --norm N3D
        -> Generates only 3OA_Stereo_N3D.txt
...
Notes:
- This builds a 3rd-order (16 channels) decoder to 2 loudspeakers, which is
  highly underdetermined; np.linalg.pinv returns a least-squares solution.
...
# Stereo speaker array at ±45° (Left, Right)
speakers = [
    [np.pi / 4, 0],        # Left  (+45°)
    [7 * np.pi / 4, 0],    # Right (-45°)
]
...
def write_decoder(norm):
    ...
    outname = f"3OA_Stereo_{norm}.txt"
...
def main():
    parser = argparse.ArgumentParser(description="Stereo 3rd-order Ambisonics decoder (SN3D/N3D).")
