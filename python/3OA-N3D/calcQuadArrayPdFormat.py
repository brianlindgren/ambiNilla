...
"""
3rd-Order Ambisonics Quad Decoder Generator
-------------------------------------------
Generates a pseudo-inverse decoding matrix for a 4-speaker horizontal quad array,
using ACN channel ordering with either N3D (orthonormalized) or SN3D (Schmidt
semi-normalized) spherical harmonics.

USAGE:
    python ambi_decoder_quad.py
        -> Generates both 3OA_Quad_SN3D.txt and 3OA_Quad_N3D.txt

    python ambi_decoder_quad.py --norm SN3D
        -> Generates only 3OA_Quad_SN3D.txt

    python ambi_decoder_quad.py --norm N3D
        -> Generates only 3OA_Quad_N3D.txt

FLAGS:
    --norm {SN3D, N3D}
        Selects normalization scheme:
            SN3D - Schmidt semi-normalized (AmbiX/Google VR default)
            N3D  - Orthonormalized
        If omitted, both normalizations are generated.

Notes:
- This builds a 3rd-order (16 channels) decoder to 4 loudspeakers, which is
  underdetermined; np.linalg.pinv returns a least-squares solution.
"""
...
# Quad speaker array (front-left, front-right, rear-right, rear-left)
speakers = [
    [np.pi / 4, 0],        # FL
    [7 * np.pi / 4, 0],    # FR
    [5 * np.pi / 4, 0],    # RR
...
def main():
    parser = argparse.ArgumentParser(description="Quad 3rd-order Ambisonics decoder (SN3D/N3D).")
